export function lagSeverity(lag) {
    const bytes = lag.bytes ?? 0;
    const secs = lag.seconds ?? 0;
    if (bytes > 100 * 1024 * 1024 || secs > 30)
        return "err";
    if (bytes > 1 * 1024 * 1024 || secs > 5)
        return "warn";
    return "ok";
}
export function colorForLag(lag) {
...
function clusterWidth(itemCount) {
    if (itemCount <= 0)
        return DB_WIDTH;
    if (itemCount === 1)
        return Math.max(DB_WIDTH, NODE_WIDTH);
    return itemCount * NODE_WIDTH + (itemCount - 1) * HORIZ_GAP;
}
function dbLabel(c) {
    const subtitle = c.kind === "structured"
...
export function buildTopology({ connections, snapshots, latestByStream, }) {
    const nodes = [];
    const edges = [];
    const pubNodesByName = {};
    const subRefs = [];
    ...
    const bottomRow = connections
        ...
        .map((c) => ({
        c,
        subs: snapshots[c.id].subscriptions,
    }));
    let xCursor = 0;
...
export function formatLag(lag) {
    const parts = [];
    if (lag.bytes != null)
        parts.push(formatBytes(lag.bytes));
    if (lag.seconds != null)
        parts.push(`${lag.seconds.toFixed(2)}s`);
